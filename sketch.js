...
function showBoats() {  
  if (boats.length > 0) {
    if (
      ...
      var positions = [-60, -100, -110, -80]; //130 is taking boat in air, so made it 60
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken ) {
        if (!pirateLaughed) {pirateLaughSound.play();pirateLaughed = true;}
        gameOver();
        isGameOver = true;
